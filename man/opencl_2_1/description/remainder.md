...
Computes the value `r` such that `r` = `x` - `n`\*`y`, where `n` is the
integer nearest the exact value of `x`/`y`. If there are two integers
closest to `x`/`y`, `n` shall be the even one. If `r` is zero, it is
