...
Returns `y` if `y` less than `x`, otherwise it returns `x`. If one
argument is a NaN, `fmin()` returns the other argument. If both
arguments are NaNs, `fmin()` returns a NaN.

## Notes

`fmin` and [`fmax`](fmax.html) behave as defined by C99 and may not
