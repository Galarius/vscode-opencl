...
Reserve `num_packets` entries for writing to pipe `p`. Returns a valid
...
-   Calls to [`read_pipe`](read_pipe.html) that takes a reservation ID
    ...
    [`reserve_write_pipe`](#). The reservation ID cannot be passed to
    another kernel including child kernels.
