...
`sincos` computes sine and cosine of `x`. The computed sine is the
return value and computed cosine is returned in `cosval`.

`sinh` computes the hyperbolic sine.

`sinpi` computes `sin` (`pi * x`).

`half_sin` computes sine. `x` must be in the range -2<sup>16</sup>…​
+2<sup>16</sup>.
