...
For each element in `v`, the bits are shifted left by the number of bits
given by the corresponding element in `i` (subject to usual shift modulo
rules). Bits shifted off the left side of the element are shifted back
in from the right.
