...
The `remquo` function computes the value `r` such that `r` = `x` -
`k`\*`y`, where `k` is the integer nearest the exact value of `x`/`y`.
If there are two integers closest to `x`/`y`, `k` shall be the even one.
If `r` is zero, it is given the same sign as `x`. This is the same value
that is returned by the [`remainder`](remainder.html) function.
