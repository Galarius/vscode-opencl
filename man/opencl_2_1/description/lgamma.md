...
function. The sign of the gamma function is returned in the `signp`
argument of the `lgamma_r`.

An application that wants to use `half` and `halfn` types will need to
include the `pragma OPENCL EXTENSION cl_khr_fp16 : enable` directive. If
extended with [`cl_khr_fp16`](#cl_khr_fp16.adoc%3C/mark%3E), generic
type name `gentype` also includes `half` and `half{2|3|4|8|16}`.
